...
import org.xidian.simplemapreduce.io.file.MultiDiskConfiguration;

public class MultiFile<K,V> implements Splittable<K,V>,Writable<K,V> {
	
	protected static final Logger logger = LogManager.getLogger(MultiFile.class.getName());
	...
	public static final MultiFile<Object,Object> ROOT = new MultiFile<Object,Object>();
	...
	public MultiFile(String path, boolean isFile)
			throws IOException {
	...
	public MultiFile[] listChild() {
		...
		List<MultiFile> listChildPath = new ArrayList<MultiFile>(listFiles.length);
		...
		try {
			...
			for(File file : listFiles){
				if(file.getName().startsWith(FILEPREFIX)){
					listChildPath.add(new MultiFile(sb.append(path).append("/").append(file.getName().substring(FILEPREFIX.length())).toString(),true));
					sb.delete(0, sb.length());
				}else{
					listChildPath.add(new MultiFile(sb.append(path).append("/").append(file.getName().substring(FOLDERPREFIX.length())).toString(),false));
					sb.delete(0, sb.length());
				}
		...
		return listChildPath.toArray(new MultiFile[1]);
	}
